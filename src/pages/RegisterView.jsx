...
import {
  Container, Card, Row, Col, Button, Form, Alert, Modal // [UBAH] Import Modal
...
export default function RegisterView() {
  ...
  const [modalContent, setModalContent] = useState({ title: '', msg: '', type: 'success', nextData: null });
    ...
    <Container className="py-5">
          ...
          <Row>
            <Col md={7} className="border-end pe-md-5">
                ...
                <div className="d-flex justify-content-between mb-4">
                    ...
                    <span className="h4 fw-bold text-primary">Rp {total.toLocaleString("id-ID")}</span>
                </div>
            ...
            <Col md={5} className="ps-md-4 mt-4 mt-md-0">
               ...
               <Form onSubmit={handleProcess}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Text className="text-muted">Opsional. Jika kosong, status invoice: <strong>Unpaid</strong>.</Form.Text>
                </Form.Group>
                <div className="d-grid gap-2">
                  <Button type="submit" variant="success" size="lg" disabled={loading}>
                    {loading ? "Memproses..." : file ? "Kirim Bukti & Bayar" : "Bayar Nanti"}
                  </Button>
      ...
      <Modal show={showModal} onHide={handleCloseModal} centered backdrop="static" keyboard={false}>
        <Modal.Body className="text-center p-5">
            <div className={`mx-auto mb-4 p-3 rounded-full w-fit ${modalContent.type === 'success' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                {modalContent.type === 'success' ? <CheckCircle size={48} /> : <XCircle size={48} />}
            </div>
